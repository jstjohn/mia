...
void add_fs( FragSeqP cfs, FragSeqP fs ) {
  ...
  if ( fs->qss == NULL ) {
    for( i = 0; i < cfs->seq_len; i++ ) {
      switch( fs->seq[i] ) {
      case 'A' :
	cfs->qss->Aqualsum[i] += fs->qual[i] - 33;
	break;
      case 'C' :
	cfs->qss->Cqualsum[i] += fs->qual[i] - 33;
	break;
      case 'G' :
	cfs->qss->Gqualsum[i] += fs->qual[i] - 33;
	break;
      case 'T' :
	cfs->qss->Tqualsum[i] += fs->qual[i] - 33;
	break;
      }
    }
  }
    ...
    for( i = 0; i < cfs->seq_len; i++ ) {
      cfs->qss->Aqualsum[i] += fs->qss->Aqualsum[i];
      cfs->qss->Cqualsum[i] += fs->qss->Cqualsum[i];
      cfs->qss->Gqualsum[i] += fs->qss->Gqualsum[i];
      cfs->qss->Tqualsum[i] += fs->qss->Tqualsum[i];
    }
    free( cfs->qss );
  }
...
void collapse_FSDB( FSDB fsdb, int Hard_cut, 
  ...
  QSSP qss; // place to put quality sums
