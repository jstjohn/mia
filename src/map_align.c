...
inline int alnseq_ol_reg(AlnSeqP as, const int rs, const int re) {
    ...
    /*
   	int fs = as->start; // fragment start coordinate
	int fe = as->end; // fragment end coordinate

	// Does fragment end before the region starts?
        // or
	// Does fragment start after region ends?
	if ((fe < rs) || (fs > re))
            return 0;
        else
            /* If neither of the above are the case, then there must
            be some overlap
            return 1;*/
}
