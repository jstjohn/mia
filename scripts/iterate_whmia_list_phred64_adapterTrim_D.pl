...
use Getopt::Std;
use vars qw( $opt_r $opt_L $opt_t $opt_H $opt_m $opt_k $opt_a $opt_w $opt_M );
use strict;
...
$cmd = "$opt_w -L $opt_L -t $opt_t -r $opt_r -6 -a $opt_a -H $opt_H -m $opt_m -k $opt_k";
$status = system( $cmd );
if ( $status ) {
...
while( !&convergence( $opt_m ) &&
    ...
    # Reassemble
    $cmd = "$opt_w -r CONS.$ITER_NUM.fa -t $opt_t -6 -L $opt_L -H $opt_H -m $opt_m -k $opt_k";
    $status = system( $cmd );
    if ( $status ) {
	print STDERR ( "$status from\n: $cmd\n" );
    }
...
sub init {
	...
	print( "-a [adapter sequence required]\n" );
