...
$cmd = "$opt_w -f $opt_f -6 -r $opt_r -H $opt_H -m $opt_m -k $opt_k";
$status = system( $cmd );
if ( $status ) {
...
while( !&convergence( $opt_m ) &&
    ...
    # Reassemble
    $cmd = "$opt_w -r CONS.$ITER_NUM.fa -f $opt_f -6 -H $opt_H -m $opt_m -k $opt_k";
    $status = system( $cmd );
    if ( $status ) {
	print STDERR ( "$status from\n: $cmd\n" );
    }
